...
/**
 * Dependency-free inline SVG line chart separating regular expense from
 * asset maintenance cost across months. Local to reports because the shared
 * AreaChart component is hardcoded to income/expense series.
 */
export function TrendChart({ data, className }: TrendChartProps) {
    ...
    const maxValue = useMemo(() => {
        const max = data.reduce(
            (m, d) => Math.max(m, d.expense, d.maintenance),
            0,
        );
    ...
    const linePath = (key: 'expense' | 'maintenance') =>
        data
        ...
        <div className={cn('flex flex-col gap-3', className)}>
            ...
            <div className="relative">
                <svg
                    ...
                    role="img"
                    aria-label="Expense vs asset maintenance over time"
                ...
                {hovered && hoverIndex !== null && (
                    <div
                        ...
                        style={{
                            ...
                            top: `${(Math.min(yFor(hovered.expense), yFor(hovered.maintenance)) / HEIGHT) * 100}%`,
                        }}
                    >
                        ...
                        <div className="text-amber-500">
                            Maintenance {formatCurrency(hovered.maintenance)}
                        </div>
                    </div>
                )}
            </div>
